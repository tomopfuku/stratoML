...
if __name__ == "__main__":
    ...
    for line in open(sys.argv[1],"r"):
        ...
        Y = np.linspace(0.305, 0.95, 10)

        # normal real values
        X = np.linspace(0.0, 0.4, 10)
        Y = np.linspace(0.005, 0.15, 10)
        print(X,Y)
        ...
        for yi in Y: 
            ...
            for xi in X:
                ...
                #start_rates = np.array([yi, 0.07, 0.6/10., xi/10.])
                start_rates = np.array([0.097, yi, 0.6/10., xi/10.])
                ll = -glc_bd.evaluate_m_l_jump(start_rates, tree, qmats, lam_mats, ss, np.array(pqr_start))
                print(start_rates, ll)
                if ll > highest:
                    highest = ll
                    highest_vals = [xi, yi]
                y_ll.append(ll)

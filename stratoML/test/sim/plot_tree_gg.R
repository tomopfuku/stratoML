...
p=p+geom_rect(aes(xmin=Start,xmax=End,ymin=ymin,ymax=ymax),fill="dark grey")+theme_bw()+geom_text(aes(x=tab$text_pos_x,y=tab$text_pos_y),hjust=-0.01,label=tab$Name,size=5.2,fontface="italic") #+ geom_text(aes(x=Start-((Start-End)/5),y=ymin - 0.6),hjust=-0.01,label=tab$support,size=3.9)
...
p=p+scale_x_reverse()
p=p+xlim(max(tab$Start),min(tab$text_pos_x)-1.01)

...
def eval_likelihood(p):
    return log_probability(p, tree, qmats, lam_mats, ss, pqr_start)

# Custom backend class to mimic emcee's expected pool behavior
class JoblibPool:
    def map(self, func, tasks):
        # uses loky backend by default to robustly ship your Cython states
        return Parallel(n_jobs=4)(delayed(func)(task) for task in tasks)

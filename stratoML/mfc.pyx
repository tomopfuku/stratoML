...
cdef double split_loglike_single_trait_marg(node.Node n, qmat.Qmat qmats, int cur_k, int chari):
    ...
    if dt < 0.0000001:
        print(n.label, ch.label, dt)
   
    #print(n.label,ch.label,list(ch1_tr),list(ch2_tr))
    # NEED TO TEST THESE ONCE I CAN GET THROUGH FIRST BRANCH
    if ch.istip:
        if len(ch.children) > 0 and ch.midpoint_lv_index != len(ch.children):
            dt = ch.lower - ch.children[0].lower
    ...
    p2 = qmats.calc_single_p_mat(dt, cur_k) # P matrix for segment along the child leading to the first likelihood vector
    ...
    for ti in range(len(cur_scen)):
        ...
        #print(list(ch1_tr),list(ch2_tr))
        n.timeslice_lv[0][chari][ti] = anclike
    if n.parent != None:
        n.scaling_factors[0][chari] = max(n.timeslice_lv[0][chari]) # update scaling factor vector
        #print("SPLITLGOLIG SINGLE MARG MAX SCALE ERROR",chari)
        #print(list(n.timeslice_lv[0][chari]))
        try:
            n.timeslice_lv[0][chari] = max_scale_timeslice(n.timeslice_lv[0][chari])
        except:
            print(n.timeslice_lv[0])
            print("ERROR SCALING LIKELIHOOD VECTOR")
            print(n.label)
            for ch in n.children:
                print(ch.label)
            print(n.get_newick_repr())
            print("SPLIT CheCK!",list(n.timeslice_lv[0][chari]))
            print(list(n.timeslice_lv[0][chari]))
            print(list(n.timeslice_lv[-1][chari]))
            print("COUNT SCEN",count)
            sys.exit()
...
cdef double split_like_marg(node.Node n, qmat.Qmat qmats, long[:] ss):
    ...
    for chari in range(1,len(ss)):
        ...
        if cur_k == 1:
            continue
        #try:
        split_loglike_single_trait_marg(n, qmats, cur_k, chari)
        #except:
        #    print("EXCEPTION CALCULATING SPLIT LOG LIKE")
        #    sys.exit()
    #sys.exit()
...
cdef budd_loglike_single_trait_marg(node.Node n, qmat.Qmat qmats, int cur_k, int chari): #, double desc_weight):
    ...
    for chd_i in reversed( range( len(n.children) ) ): # start with child farthest from the root
        ...
        #if ch.istip:
        if len(ch.children) > 0 and ch.midpoint_lv_index != len(ch.children):
            #print(ch.lower,[ch1.lower for ch1 in ch.children])
            #sys.exit()
            dt = ch.lower - ch.children[0].lower
        elif len(ch.children) > 0 and ch.midpoint_lv_index == len(ch.children) or len(ch.children) == 0:
            dt = ch.lower - ch.midpoint
        ...
        if dt < 0.0000001:
            print("SPOT2", n.label, ch.label, dt)
            print(list(prev_ch))
            print(list(prev_ls))
            print(prev_time, n.midpoint_lv_index,n.midpoint)
            print(n.label,n.lower,n.upper)
            print(ch.label,ch.lower,ch.upper)
            sys.exit()
        ...
        for ancst in ancsts:
            ...
            for j in range(len(inher)): # calc cond like for each allowed inheritance scen
                ...
                if startst == 0:
                    break
                #scenario_like = 0.0 # partial likelihood of one scenario, marginalized over all combinations of anagenetic transitions and budded descendant states
        ...
        #print("ERROR INBOUND")
        #print("num scaling factors", len(n.scaling_factors))
        #print(list(n.scaling_factors[ch.parent_lv_index]))
        ...
        try:
            ...
            for ch in n.children:
                print(ch.label)
                print(list(ch.timeslice_lv[0][0]))
            print(n.get_newick_repr())
            print("BUDD CheCK!",list(n.timeslice_lv[0][chari]))
            print(dt)
            print(list(n.timeslice_lv[0][chari]))
            print(list(n.timeslice_lv[-1][chari]))
            sys.exit()
        ...
        prev_time = ch.lower
        #print(n.label,list(n.timeslice_lv[0][0]))
        #print(n.label,cur_k) 
        #print(n.label,list(n.disc_traits[0]))
...
def calc_ASR_down_node(node.Node n, qmat.Qmat qmats, long[:] ss, bint do_inher = True):
    cdef node.Node ch
    cdef double[:,:] p1
    ...
    for ch_i in range(len(n.children)):
        ...
        compute_budd_ASR_one_step(dt, qmats, ss)

def compute_budd_ASR_one_step(double dt, qmat.Qmat qmats, long[:] ss):
    cdef int i, cur_k
    cdef double[:, :, :] pmats

    pmats = qmats.calc_p_mats(dt)
    print(list(pmats[0][1]))
    sys.exit()
...
def compute_mfc2_ASRs(node.Node tree, qmat.Qmat qmats, long[:] ss):
    cdef node.Node n, ch
    cdef double[:,:] root_marg_likes, curlike, p1
    ...
    for n in tree.iternodes(0):
        if n == tree:
            if n.istip == False:
                ...
                if n.children[0].lower > n.midpoint:
                    dt = n.lower - n.children[0].lower
                    p1 =  qmats.calc_single_p_mat(dt, cur_k) 
                    trait_marg = compute_anagenetic_marg(p1, root_marg_likes, n.timeslice_lv[-1])
                    n.timeslice_lv[-1] = trait_marg
        if n.istip == False:
                splitting_forward_probs(n, qmats, ss)
        else:
            calc_ASR_down_node(n, qmats,ss,True)
...
def splitting_forward_probs(node.Node n, qmat.Qmat qmats,  long[:] ss):
    ...
    for i in range(1,len(n.timeslice_lv[0])): # i is a trait index
        if ss[i] == 1:
            continue
        splitting_forward_probs_single_trait(n, qmats, ss[i], i)
...
def splitting_forward_probs_single_trait(node.Node n, qmat.Qmat qmats, int cur_k, int chari):
    ...
    #print("f1", n.children[0].label, forward_probs1) 
    #print("f2", n.children[1].label, forward_probs2)
...
cdef double calc_invar_ll_marg(node.Node tree, qmat.Qmat qmats):
    cdef double desc_weight, nodell, treell, sum_log_sf, sublike, sum_plikes, plike
    cdef double[:] plikes
    cdef node.Node n
    cdef int i, j, count
    ...
    for n in tree.iternodes(1):
        if len(n.children) == 0:
            continue
        if n.istip == False:
            split_loglike_single_trait_marg(n, qmats, 2, 0)
        elif n.istip:
            budd_loglike_single_trait_marg(n, qmats, 2, 0)
            #print(n.label,list(n.timeslice_lv[-1][0]),n.scaling_factors[-1][0])
